...
namespace _06042020
{
    static class ListExten
    {
        public static void Print<T>(this List<T> list, string name, string rn = "\n")
        {
            ...
            Console.WriteLine(new string('-', 7* list.Count()));
        ...
        public static void Print<Tkey, Tvalue>(this Dictionary<Tkey, Tvalue> dic, string name, string rn = "\n")
        {
            Console.WriteLine(name);
            Console.WriteLine(new string('-', 20));
            foreach (var item in dic)
            {
                Console.Write($"|{item.Key,-15}|{item.Value,10}{rn}");
            }
            Console.WriteLine();
